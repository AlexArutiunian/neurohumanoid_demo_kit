...
public:
    ...
    {
        // Modbus TCP 设置
        std::string ip_address = "192.168.123.211"; 
        int port = 6000; 
                       ...
                       const service_interfaces::srv::Setangle::Response::SharedPtr response)
{
        ...
        (request->angle5 >= -1 && request->angle5 <= 1000))
    {
        ...
        response->angle_accepted = (modbus_write_registers(ctx_, 1486, 6, angles) == 0); // 写入所有角度

        // 读取某个寄存器的值（小拇指角度）
        uint16_t read_value[1];
        if (modbus_read_registers(ctx_, 1486, 1, read_value) != -1) {
            RCLCPP_INFO(this->get_logger(), "Read angle0 value: %d", read_value[0]);
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to read angle0 value: %s", modbus_strerror(errno));
        }
    }
    else
    {
                         ...
                         const service_interfaces::srv::Setpos::Response::SharedPtr response)
{
        ...
        (request->pos5 >= 0 && request->pos5 <= 2000))
    {
        ...
        response->pos_accepted = (modbus_write_registers(ctx_, 1474, 6, positions) == 0); // 写入所有位置

        // 读取某个寄存器的值（位置0）
        uint16_t read_value[1];
        if (modbus_read_registers(ctx_, 1474, 1, read_value) != -1) {
            RCLCPP_INFO(this->get_logger(), "Read position0 value: %d", read_value[0]);
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to read position0 value: %s", modbus_strerror(errno));
        }
    }
    else
    {
        ...
        (request->speed5 >= 0 && request->speed5 <= 1000))
    {
        ...
        response->speed_accepted = (modbus_write_registers(ctx_, 1522, 6, speeds) == 0); // 写入所有速度

        // 读取某个寄存器的值（小拇指速度）
        uint16_t read_value[1];
        if (modbus_read_registers(ctx_, 1522, 1, read_value) != -1) {
            RCLCPP_INFO(this->get_logger(), "Read speed0 value: %d", read_value[0]);
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to read speed0 value: %s", modbus_strerror(errno));
        }
    }
    else
    {
        ...
        (request->force5 >= 0 && request->force5 <= 3000))
    {
        ...
        response->force_accepted = (modbus_write_registers(ctx_, 1498, 6, forces) == 0); // 写入所有力控设置值

        // 读取某个寄存器的值（小拇指力控设置值）
        uint16_t read_value[1];
        if (modbus_read_registers(ctx_, 1498, 1, read_value) != -1) {
            RCLCPP_INFO(this->get_logger(), "Read FORCE_SET(0) value: %d", read_value[0]);
        } else {
            RCLCPP_ERROR(this->get_logger(), "Failed to read FORCE_SET(0) value: %s", modbus_strerror(errno));
        }
    }
    else
    {
                       ...
                       const service_interfaces::srv::Setid::Response::SharedPtr response)
{
    ...
    // 检查请求中的 ID 是否在合法范围内
    if (request->setid >= 1 && request->setid <= 254) {
        // 将 ID 写入 Modbus 寄存器
        uint16_t id_value = static_cast<uint16_t>(request->setid);
        response->idgrab = (modbus_write_registers(ctx_, 1000, 1, &id_value) == 0); // 写入 ID
        ...
        if (response->idgrab) {
            ...
            // 读取寄存器以验证写入的 ID
            uint16_t read_value;
            if (modbus_read_registers(ctx_, 1000, 1, &read_value) != -1) {
                RCLCPP_INFO(this->get_logger(), "Read ID from Modbus register: %d", read_value);
                if (read_value == request->setid) {
                    RCLCPP_INFO(this->get_logger(), "ID verification successful.");
                } else {
                    RCLCPP_WARN(this->get_logger(), "ID verification failed! Expected: %d, Read: %d", request->setid, read_value);
            ...
            RCLCPP_ERROR(this->get_logger(), "Failed to write ID to Modbus register.");
