...
python3 g1_json_hands.py --dataset-path ./216.json --enable-wrist-py
...
class Custom:
    def __init__(self, *, enable_wrist_pitch_yaw=False):
        self.time_ = 0.0
    ...
    def LoadDataset(
        ...
        *,
            ...
            for item in frame:
                if isinstance(item, dict) and isinstance(item.get("name"), str):
                    ...
                    if name not in JOINT_NAME_TO_INDEX:
                        ...
                        # талию из датасета игнорируем всегда (держим как было)
        ...
        if ignored:
            print(
                "WARNING: Some joints from dataset are ignored (unknown / not controlled / waist locked by hold):\n"
                + "\n".join(sorted(ignored))
            )
    ...
    def LowStateHandler(self, msg: LowState_):
        ...
        if self.q_start_arms is None:
            ...
            # 2) Собираем poses из keyframes (только руки)
            for kf in self.keyframes:
                ...
                for item in kf.get("frame", []):
                    ...
                    # талию намеренно игнорируем (держим как было)
    ...
    def _apply_waist_hold(self):
        """Талия всегда в углах, которые были на старте."""
        ...
        for j in WAIST_JOINTS:
            ...
            self.low_cmd.motor_cmd[j].q = float(self.waist_hold_q[j])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kp-waist", type=float, default=120.0, help="Waist kp (hold)")
    parser.add_argument("--kd-waist", type=float, default=4.0, help="Waist kd (hold)")
    ...
    custom = Custom(enable_wrist_pitch_yaw=args.enable_wrist_py)
